...
import {CreateModuleClassMatcher, ScaleImage} from "@/utils/Utils.js";
    ...
    >
      {
        (aiStore.searchResults.results || []).map((result, index) =>
          <Component
            listItem={showList}
            key={`result-${index}`}
            link={UrlJoin("/", "search", queryB58, index.toString())}
            label={result.name}
            aspectRatio={aiStore.searchResults.isAssetSearch ? "square" : "landscape"}
...
const SearchResults = observer(() => {
  ...
  }, [queryB58, aiStore.selectedSearchIndexId]);
