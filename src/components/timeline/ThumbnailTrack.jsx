...
const ThumbnailCreationTrack = observer(() => {
  let content;
  switch(videoStore.thumbnailStore.thumbnailStatus?.status?.state) {
    case "started":
    case "running":
      content = (
        <div className={S("thumbnail-creation-track__status")}>
          <div>Generating Thumbnails...</div>
          <Progress w={200} value={videoStore.thumbnailStore.thumbnailStatus?.status?.progress || 0} max={100}/>
        </div>
      );
      break;
        ...
        <Button
          ...
          onClick={() =>
            modals.openConfirmModal({
              ...
              labels: { confirm: "Generate Thumbnails", cancel: "Cancel" },
              onConfirm: () => videoStore.thumbnailStore.GenerateVideoThumbnails()
            })
          }
        >
          Generate Thumbnails
  ...
  useEffect(() => {
    if(!["running", "started"].includes(videoStore.thumbnailStore.thumbnailStatus?.status?.state)) {
      return;
    }
  ...
  }, [videoStore.thumbnailStore.thumbnailStatus?.status?.state]);
...
const ThumbnailTrack = observer(() => {
  ...
  if(!videoStore.thumbnailStore.thumbnailStatus.available) {
    ...
    >
      <div
        ...
        onMouseMove={event => {
          ...
          setHoverThumbnail(videoStore.thumbnailStore.ThumbnailImage(progress * videoStore.duration / 100));
        }}
        ...
        >
          <div className={S("thumbnail-track__cover")} />
          {
            [...new Array(visibleThumbnails)].map((_, index) => {
              ...
              return (
                <img
                  src={videoStore.thumbnailStore.ThumbnailImage(startTime)}
                  alt="Thumbnail"
                  key={`thumbnail-${index}`}
                  style={{
