...
class TrackWorker {
  ...
  Line(imageData, color, sx, sy, ex, ey) {
    for(let x = sx; x <= ex; x++) {
      for(let y = sy; y <= ey; y++) {
        this.SetPixel(imageData, color, x, y);
      }
    }
  }

  Rect(imageData, x, y, width, height, color) {
    this.Line(imageData, color, x, y, x + width, y);
    this.Line(imageData, color, x + width, y, x + width, y + height);
    this.Line(imageData, color, x, y, x, y + height);
    this.Line(imageData, color, x, y + height, x + width, y + height);
  }
